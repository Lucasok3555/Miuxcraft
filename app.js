...
function mergeBoxGeometries(positions) {
  const srcPos = blockGeo.attributes.position.array;
  const srcNorm = blockGeo.attributes.normal.array;
  const srcUv = blockGeo.attributes.uv.array;
  const srcIndex = blockGeo.index.array;
  const vertsPerBox = srcPos.length / 3;
  const idxPerBox = srcIndex.length;
  const n = positions.length;
  const outPos = new Float32Array(n * vertsPerBox * 3);
  const outNorm = new Float32Array(n * vertsPerBox * 3);
  const outUv = new Float32Array(n * vertsPerBox * 2);
  const outIndex = new Uint32Array(n * idxPerBox);
  for (let i = 0; i < n; i += 1) {
    const { x, y, z } = positions[i];
    const posOffset = i * vertsPerBox * 3;
    for (let v = 0; v < vertsPerBox; v += 1) {
      outPos[posOffset + v * 3] = srcPos[v * 3] + x;
      outPos[posOffset + v * 3 + 1] = srcPos[v * 3 + 1] + y;
      outPos[posOffset + v * 3 + 2] = srcPos[v * 3 + 2] + z;
    }
    outNorm.set(srcNorm, i * vertsPerBox * 3);
    outUv.set(srcUv, i * vertsPerBox * 2);
    const idxOffset = i * idxPerBox;
    const vertBase = i * vertsPerBox;
    for (let j = 0; j < idxPerBox; j += 1) {
      outIndex[idxOffset + j] = srcIndex[j] + vertBase;
    }
  }
  ...
  geo.setIndex(new THREE.BufferAttribute(outIndex, 1));
  return geo;
}

function registerBlock(x, y, z, type, group) {
  const def = blockDef(type);
  if (!def) return;
  if (!group.userData.blocksByType) group.userData.blocksByType = new Map();
  let bucket = group.userData.blocksByType.get(type);
  if (!bucket) {
    bucket = new Map();
    group.userData.blocksByType.set(type, bucket);
  }
  bucket.set(key(x, y, z), { x: x + 0.5, y: y + 0.5, z: z + 0.5 });
  loadedBlocks.set(key(x, y, z), type);
  if (def.light) addLightIfNeeded(x, y, z, type);
  addBlockCollider(x, y, z, type);
}

function rebuildChunkTypeMesh(group, type) {
  if (!group.userData.meshByType) group.userData.meshByType = new Map();
  const oldMesh = group.userData.meshByType.get(type);
  if (oldMesh) {
    group.remove(oldMesh);
    oldMesh.geometry.dispose();
    const idx = blockMeshes.indexOf(oldMesh);
    if (idx >= 0) blockMeshes.splice(idx, 1);
    group.userData.meshByType.delete(type);
  }
  const bucket = group.userData.blocksByType?.get(type);
  if (!bucket || bucket.size === 0) return;
  const def = blockDef(type);
  const blockList = Array.from(bucket.values());
  const merged = mergeBoxGeometries(blockList);
  const mesh = new THREE.Mesh(merged, materialFor(def));
  mesh.castShadow = Boolean(def.solid);
  mesh.receiveShadow = true;
  mesh.userData = { type, blockList };
  group.add(mesh);
  group.userData.meshByType.set(type, mesh);
  blockMeshes.push(mesh);
}

function addMesh(x, y, z, type, group, deferred) {
  const def = blockDef(type);
  if (!def) return;
  registerBlock(x, y, z, type, group);
  if (!deferred) rebuildChunkTypeMesh(group, type);
}
...
function generateTree(x, y, z, group) {
  ...
  for (let i = 0; i < h; i += 1) addMesh(x, y + i, z, 'wood', group, true);
  for (let ox = -2; ox <= 2; ox += 1) {
    for (let oz = -2; oz <= 2; oz += 1) {
      for (let oy = h - 2; oy <= h; oy += 1) {
        if (Math.abs(ox) + Math.abs(oz) + Math.max(0, oy - h + 1) < 4) {
          addMesh(x + ox, y + oy, z + oz, 'leaf', group, true);
        }
      }
    }
  }
...
function generateFallenLog(x, y, z, group) {
  ...
  for (let i = 0; i < len; i += 1) {
    addMesh(x + (alongX ? i : 0), y, z + (alongX ? 0 : i), 'wood', group, true);
  }
}
...
function flushChunkMeshes(group) {
  if (!group.userData.blocksByType) return;
  for (const type of group.userData.blocksByType.keys()) rebuildChunkTypeMesh(group, type);
}
...
function loadChunk(cx, cz) {
  ...
  if (cached) {
    for (const entry of cached.blocks) addMesh(entry[1], entry[2], entry[3], entry[0], group, true);
    for (const entry of cached.plants || []) addPlant(entry[0], entry[1], entry[2], group, entry[3]);
    flushChunkMeshes(group);
    chunks.set(ck, group);
  ...
  for (let lx = 0; lx < CHUNK_SIZE; lx += 1) {
    for (let lz = 0; lz < CHUNK_SIZE; lz += 1) {
      ...
      for (let y = start; y <= Math.max(h, 15); y += 1) {
        ...
        if (type && isSurfaceVisible(x, y, z)) {
          addMesh(x, y, z, type, group, true);
          blocksForCache.push([type, x, y, z]);
        }
      }
      const biome = BIOMES[biomeAt(x, z)];
  ...
  flushChunkMeshes(group);
  const cacheBlocks = [];
  for (const [type, bucket] of group.userData.blocksByType) {
    for (const pos of bucket.values()) cacheBlocks.push([type, Math.floor(pos.x), Math.floor(pos.y), Math.floor(pos.z)]);
  }
  writeChunkCache(cx, cz, { blocks: cacheBlocks, plants });
  chunks.set(ck, group);
  worldGroup.add(group);
}
...
function unloadChunk(ck) {
  ...
  if (!group) return;
  if (group.userData.blocksByType) {
    for (const bucket of group.userData.blocksByType.values()) {
      for (const pos of bucket.values()) {
        const bx = Math.floor(pos.x);
        const by = Math.floor(pos.y);
        const bz = Math.floor(pos.z);
        loadedBlocks.delete(key(bx, by, bz));
        removeLightIfAny(bx, by, bz);
        removeBlockCollider(bx, by, bz);
      }
    }
  }
  if (group.userData.meshByType) {
    for (const mesh of group.userData.meshByType.values()) {
...
function refreshBlockMesh(x, y, z) {
  ...
  if (oldType && (!visible || oldType !== type)) {
    if (group) group.userData.blocksByType?.get(oldType)?.delete(k);
    removeLightIfAny(x, y, z);
    removeBlockCollider(x, y, z);
    loadedBlocks.delete(k);
  ...
  if (!loadedBlocks.has(k) && visible && group) {
    addMesh(x, y, z, type, group);
  }
}
...
function findTarget() {
  ...
  for (const hit of hits) {
    ...
    if (!list || hit.faceIndex == null) continue;
    const pos = list[Math.floor(hit.faceIndex / 12)];
    if (!pos) continue;
    return { point: hit.point, face: hit.face, blockType: hit.object.userData.type, blockPos: { x: Math.floor(pos.x), y: Math.floor(pos.y), z: Math.floor(pos.z) } };
  }
  return null;
}
...
function interact(place) {
  ...
  if (!hit) return;
  if (!place) {
    createDrop(hit.blockPos.x, hit.blockPos.y, hit.blockPos.z, hit.blockType);
    setBlock(hit.blockPos.x, hit.blockPos.y, hit.blockPos.z, null);
    return;
  }
...
function clearLoadedWorld() {
  for (const mesh of blockMeshes) mesh.geometry.dispose();
  worldGroup.clear();
  weatherGroup.clear();
  dropGroup.clear();
  chunks.clear();
