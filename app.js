...
const HOTBAR = ['grass', 'stone', 'wood', 'sand', 'glass', 'water', 'brick', 'diamond_block', 'furnace'];
...
function heightAt(x, z) {
  ...
  return Math.floor(8 + base * 21 + detail * 4 + rolling + mountain + island);
}
...
function interact(place) {
  ...
  if (!blockDef(selectedItem)?.solid && selectedItem !== 'water' && selectedItem !== 'lava') return;
...
function renderWorlds() {
  ...
  for (const world of worlds) {
    ...
    meta.innerHTML = `<strong>${world.name}</strong><small>${world.mode === 'creative' ? 'Criativo' : 'Sobrevivencia'} | seed ${world.seed}</small>`;
    ...
    play.textContent = 'Entrar';
    play.onclick = () => startWorld(world.id);
...
async function startWorld(id) {
  ...
  el.topWorld.textContent = `${currentWorld.name} | ${currentWorld.mode === 'creative' ? 'Criativo' : 'Sobrevivencia'}`;
  running = false;
...
async function shareCurrentWorld() {
  ...
  if ('RTCPeerConnection' in window) {
    try {
      ...
      channel.onopen = () => {
        ...
        el.shareStatus.textContent = 'Conexao WebRTC aberta.';
      };
  ...
  el.shareStatus.textContent = payload.offer ? 'Codigo pronto. Envie para o outro jogador.' : 'Codigo pronto com dados do mundo; WebRTC nao ficou disponivel neste navegador.';
}
...
async function joinOnlineWorld() {
  ...
  const meta = document.createElement('div');
  meta.innerHTML = `<strong>${payload.world.name}</strong><small>Host ${payload.host || 'Jogador'} | ${payload.world.mode === 'creative' ? 'Criativo' : 'Sobrevivencia'}</small>`;
  const actions = document.createElement('div');
  actions.className = 'world-actions';
  const play = document.createElement('button');
  ...
  el.onlineStatus.textContent = onlineSession?.answer ? 'Mundo encontrado. Resposta WebRTC criada localmente para teste.' : 'Mundo encontrado pelo codigo.';
}
...
el.mobileBreak.onclick = () => interact(false);
